...
async def ratio(ctx, main: int, ratio: int, max_size: int):
	...
	sentense_inf = f"{inf_heigh_t:<8,}死役:  {inf_low_t:<8,}  |  合計:  {inf_heigh_t+inf_low_t:>8,}"
	sentense_range = f"{range_heigh_t:<8,}死役:  {range_low_t:<8,}  |  合計:  {range_heigh_t+range_low_t:>8,}"
	sentense_cav = f"{cav_heigh_t:<8,}死役:  {cav_low_t:<8,}  |  合計:  {cav_heigh_t+cav_low_t:>8,}"
